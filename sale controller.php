    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['reference_no'] = $sale->staff_note;
                $nestedData['biller'] = "Biller";
    ...
    public function store(Request $request)
    {
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            if($paying_method == 'Credit Card'){
                ...
                $lims_customer_data->payment_method = "Credit Card";
    ...
    public function genInvoice($id)
{
    ...
    if ($lims_sale_data->staff_note == null) {
        // Generate Invoice Data
        $payment_mode = collect($lims_payment_data)->contains('paying_method', 'Cash') ? 1 : 2;
        $invoice_items = [];
        $total_quantity = 0;
        $total_discount = 0;

        foreach ($lims_product_sale_data as $product_sale) {
            $product = \App\Product::find($product_sale->product_id);
            $discount_value = ($product_sale->discount / 100) * $product->price;
            $price_after_discount = $product->price - $discount_value;

            $invoice_items[] = [
                'ItemCode' => $product_sale->product_id,
                ...
                'TotalAmount' => $price_after_discount + ($price_after_discount * 0.18),
                
                'TaxCharged' => $price_after_discount * 0.18,
                'InvoiceType' => 1,
                'Discount' => $discount_value,
                'RefUSIN' => null
            ];

            $total_quantity += $product_sale->qty;
            $total_discount += $discount_value;
        }

        $invoice_data = [
            'InvoiceNumber' => $lims_sale_data->reference_no,
            'POSID' => 152404,
            'USIN' => 'USIN0',
            'DateTime' => now(),
            'BuyerName' => $lims_customer_data->name,
            'BuyerPhoneNumber' => $lims_customer_data->phone_number,
            'Items' => $invoice_items,
            
            'TotalBillAmount' => $lims_sale_data->grand_total + $lims_sale_data->grand_total * 0.17,
            'TotalQuantity' => $total_quantity,
            
            'TotalSaleValue' => $lims_sale_data->total_price ,
            // 'TotalSaleValue' => $lims_sale_data->total_price + $lims_sale_data->grand_total *0.18,
            
            'TotalTaxCharged' => $lims_sale_data->total_price * 0.17,
            // 'TotalTaxCharged' => $lims_sale_data->grand_total * 0.18,
            'Discount' => $total_discount + $lims_sale_data->order_discount,
            'PaymentMode' => $payment_mode,
            'InvoiceType' => 1
        ];

        // Post to FBR API
        $ch = curl_init('https://gw.fbr.gov.pk/imsp/v1/api/Live/PostData');
        $headers = [
        ...
        if ($error) {
            // Handle cURL error
            throw new \Exception("FBR API Error: $error");
        }

        $response_data = json_decode($response, true);
        if (!isset($response_data['InvoiceNumber'])) {
            throw new \Exception('Invalid response from FBR API: ' . $response);
        }

        $invoice_number = $response_data['InvoiceNumber'];
        $lims_sale_data->update(['staff_note' => "FBR Fiscal Invoice Number : $invoice_number"]);
    } else {
        $invoice_number = str_replace('FBR Fiscal Invoice Number : ', '', $lims_sale_data->staff_note);
    }
...
$bc = $invoice_number; // Compatibility with view

// $qr_data = [
//     'InvoiceNumber' => $invoice_number,
//     'TotalBillAmount' => $lims_sale_data->grand_total + $lims_sale_data->grand_total ,
//     'TotalTaxCharged' => $lims_sale_data->grand_total ,
//     'DateTime' => now()->toDateTimeString()
// ];
// $qr_code = json_encode($qr_data);
...
return view('sale.invoice', compact(
    ...
    public function addPayment(Request $request)
    {
        ...
        $data['payment_reference'] = 'spr-' . date("Ymd") . '-'. date("his");
        $lims_payment_data->payment_reference = $data['payment_reference'];
        $lims_payment_data->change = $data['paying_amount'] - $data['amount'];        $lims_payment_data->amount = $data['amount'];
